...
function renderFeatured() {
  ...
  grid.innerHTML = destaques.map(p => productCardHtml(p, false)).join('');
}
...
function productCardHtml(p, showBadge) {
  ...
  const badge = showBadge && p.destaque ? `<span class="product-badge">⭐ Destaque</span>` : '';
  return `
...
function renderCart() {
  ...
  if (!cart.length) {
    ...
    itemsEl.innerHTML = `<div class="cart-empty"><p>Sua sacola está vazia 🛍️</p>
      <button class="btn btn-outline-dark btn-sm" onclick="closeCart()">Ver catálogo</button></div>`;
    return;
  }
...
function syncCartButtons() {
  document.querySelectorAll('.product-card[data-id]').forEach(card => {
    ...
    const inCart = cart.some(i => i.id === card.dataset.id);
    ...
    try {
      if (CONFIG.scriptUrl) {
        await fetch(CONFIG.scriptUrl, {
          method: 'POST',
          body: JSON.stringify({ action: 'cadastro_clube', nome, whatsapp, email, nascimento: nasc }),
        });
      } else {
        const msg = ['🌿 *Cadastro — Clube da Brisa*', '', `Nome: ${nome}`, `WhatsApp: ${whatsapp}`,
          email ? `Email: ${email}` : null, nasc ? `Nascimento: ${formatDate(nasc)}` : null].filter(Boolean).join('\n');
        window.open(`https://wa.me/${CONFIG.whatsappNumber}?text=${encodeURIComponent(msg)}`, '_blank');
      }
      form.classList.add('hidden');
      document.getElementById('loyaltySuccess').classList.remove('hidden');
    } catch {
      btn.textContent = 'Cadastrar no Clube';
      btn.disabled    = false;
...
function setupPontosConsulta() {
  ...
  if (!section) return;
  section.style.display = CONFIG.scriptUrl ? '' : 'none';
}
  ...
  el.innerHTML = historico.map(h => `
    <div class="hist-item">
      <div class="hist-item-top">
        <span class="hist-produto">${escHtml(h.produto || '')}</span>
        <span class="hist-valor">R$&nbsp;${formatPrice(h.valor || 0)}</span>
      </div>
      <div class="hist-item-sub">
        ${formatDate(h.data)}${h.quantidade ? ` · ${h.quantidade}x` : ''}${h.pagamento ? ` · ${escHtml(h.pagamento)}` : ''}
      </div>
    </div>`).join('');
}
